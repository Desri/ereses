...
import FilterMasyarakatComponent from "./filter";
import { deleteUser, getMasyarakat } from "@/services/api/masyarakat";
import { deleteFraksi, putFraksi } from "@/services/api/fraksi";
import {
  Table,
  TableBody,
  TableCell,
...
export default function DashboardKelolaAkunMasyarakatComponent() {
  const [fraksi, setFraksi] = useState<User>();
  ...
  const [item, setItem] = useState({ isActive: false });
  ...
  return (
    <>
      ...
      <div className="bg-white">
        <div className="font-bold mb-3.5">Kelola Akun Masyarakat</div>
        <FilterMasyarakatComponent />
        <Table>
          <TableHeader>
            <TableRow>
